...
class SheetContext(Protocol):
    flags: SheetState = SheetState.NONE
    headings_dim: dict[str, int] = {}
    headings_hided: dict[str, int] = {}

    coords_vportq3: tuple[int, int] = (COL_CELLS_WIDTH, ROW_CELLS_HEIGHT)
    viewport_q3: tuple[int, int, int, int] = (1, 1, 1, 1)
    coords_vportq1: tuple[int, int] = coords_vportq3
    viewport_q1: tuple[int, int, int, int] = (1, 1, 1, 1)
...
class SheetContextData:
    """A concrete implementation of the SheetContext protocol."""
    def __init__(self):
        self.headings_dim: dict[str, int] = {}
        ...
        if bflag:
            ...
            self.active_cell: tuple[int, int] = self.viewport_q1[:2]
            self.selected_cells: tuple[int, int, int, int] = (*self.active_cell, *self.active_cell)
        else:
            self.flags: SheetState = SheetState.GRIDLINES | SheetState.HEADINGS | SheetState.FREEZE
            self.viewport_q3: tuple[int, int, int, int] = (1, 2, 8, 5)
            self.active_cell: tuple[int, int] = (14, 15)
            self.viewport_q1: tuple[int, int, int, int] = (12, 10, 15, 15)
            self.selected_cells: tuple[int, int, int, int] = (13, 13, 16, 18)


class SheetLook:
    slots = ('_winfo_width', '_winfo_height', '_sht_ctx')

    def __init__(self, sht_ctx: SheetContext | None=None, width: int | None=None, height: int | None=None):
        self._winfo_width = width
        self._winfo_height = height

        self.sht_ctx = sht_ctx or SheetContextData()
        pass

    @property
    def sht_ctx(self) -> SheetContext:
        return self._sht_ctx
    
    @sht_ctx.setter
    def sht_ctx(self, value: SheetContext):
        self._sht_ctx = ctx = value
        coords = ctx.coords_vportq3
        if ctx.viewport_q3[:2] != ctx.viewport_q3[2:]:
            coords = self.cell_coordinates(*ctx.viewport_q3[2:], ctx.viewport_q3, ctx.coords_vportq3)[:2]
        ctx.coords_vportq1 = coords

    def __getattr__(self, attr):
        "Delegate attribute access to the sht_ctx object"
        if attr in self.sht_ctx.__dir__():
            return getattr(self.sht_ctx, attr)
        raise AttributeError(f"'{self.__class__.__name__}' object has no attribute '{attr}'")

    def __dir__(self):
        "Include attributes from the sht_ctx object in dir()"
        # return super().__dir__() + self.sht_ctx.__dir__()
    ...
    def winfo_width(self, value):
        ...
        if value:
            ...
            ctx.viewport_q1 = (*ctx.viewport_q1[:2], xcell, ctx.viewport_q1[3])
        pass
    ...
    def winfo_height(self, value):
        ...
        if value:
            ...
            ctx.viewport_q1 = (*ctx.viewport_q1[:3], ycell)
        pass
    ...
    def efective_area(self):
        ctx = self.sht_ctx
        lt_corner_x = ctx.coords_vportq3[0] - COL_CELLS_WIDTH
        lt_corner_y = ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT
        rb_corner_x = self.tag_coords(ctx.viewport_q1[2], axis=0)[1]
        rb_corner_y = self.tag_coords(ctx.viewport_q1[3], axis=1)[1]
        return (lt_corner_x, lt_corner_y, rb_corner_x, rb_corner_y)

    @contextmanager
    def pivot_point(self, isActiveCell=False, isUp=0):
        ctx = self.sht_ctx
        acell_x0, acell_y0 = x, y = ctx.active_cell
        if not isActiveCell:
            sel_x0, sel_y0, sel_x1, sel_y1 = ctx.selected_cells
        ...
        if isActiveCell:
            ctx.active_cell = pt.x, pt.y
            ctx.selected_cells = (*ctx.active_cell, *ctx.active_cell)
        else:
            acell_x0, acell_y0 = ctx.active_cell
            ...
            ctx.selected_cells = sel_x0, sel_y0, sel_x1, sel_y1
        pass
    ...
    def tag_coords(self, tag: int, viewport:tuple[int, ...]=None, coords_viewport: tuple[int, int]=None, axis: Literal[0, 1]=0) -> tuple[int, int]:
        ...
        if viewport is None:
            viewport = ctx.viewport_q1[:2]
        if coords_viewport is None:
            coords_viewport = ctx.coords_vportq1
    ...
    def cell_quadrant(self, x: int, y:int, isCoord: bool=True) -> int:
        ...
        ctx = self.sht_ctx
        if ctx.coords_vportq1 == ctx.coords_vportq3:
            return 1
        xdiscr = ctx.coords_vportq1[0] if isCoord else ctx.viewport_q3[2]
        ydiscr = ctx.coords_vportq1[1] if isCoord else ctx.viewport_q3[3]
    ...
    def quadrant_data(self, nquadrant:int) -> tuple[tuple[int, ...], tuple[int, int]]:
        ...
        ctx = self.sht_ctx
        if nquadrant == 1:
            return ctx.viewport_q1, ctx.coords_vportq1
        elif nquadrant == 2:
            orig = ctx.viewport_q1[0], ctx.viewport_q3[1], ctx.viewport_q1[2], ctx.viewport_q3[3] - 1
            return orig, (ctx.coords_vportq1[0], ctx.coords_vportq3[1])
        elif nquadrant == 3:
            return (*ctx.viewport_q3[:2], ctx.viewport_q3[2] - 1, ctx.viewport_q3[3] - 1), ctx.coords_vportq3
        else: # nquadrant == 4
            orig = ctx.viewport_q3[0], ctx.viewport_q1[1], ctx.viewport_q3[2] - 1, ctx.viewport_q1[3]
            return orig, (ctx.coords_vportq3[0], ctx.coords_vportq1[1])
    ...
    def set_dimension(self, x0:int, x1:int, width:int, axis:Literal[0, 1]=0) -> int:
        ...
        rbcorner_vp = self.cell_containing_coords(self.efective_width(), self.efective_height())
        ctx.viewport_q1 = (*ctx.viewport_q1[:2], *rbcorner_vp)
        return delta
...
class SheetUI(tk.Canvas):
    ...
    @property
    def format_header(self) -> Callable[[int, Literal[0, 1]], str]:
    ...
    def map_cell_to_coords(self, x, y, coords=None, coords_viewport=None):
        ...
        winfo_width, winfo_height = look.efective_width(), look.efective_height()
        x = max(1, min(MAX_COLS, x))
        y = max(1, min(MAX_ROWS, y))
        linf_x, linf_y = look.cell_coordinates(x, y, viewport, coords_viewport)[:2]
        deltax, deltay = linf_x - coords_viewport[0], linf_y - coords_viewport[1]
        clinf_x, clinf_y, lsup_x, lsup_y = look.efective_area()

        ptx0 =  ptx1 = pty0 = pty1 = None
        gx1, gy1 = map(int, self.coords("background")[2:])

        if deltax and abs(deltax) >= winfo_width:
            # All the cell information and column headings need to be updated
            items = self.find_enclosed(coords_viewport[0] - 1, clinf_y - 1, gx1 + 1, gy1 + 1)
            self.delete(*items)
            viewport_x0 = x
            ...
            items = self.find_enclosed(coords_viewport[0] - 1, clinf_y - 1, linf_x + 1, gy1 + 1)
            for item in items:
                self.move(item, -dx, 0)
            
            # Resize the horizontal gridlines
            if gx1 != lsup_x:
                for item in self.find_withtag("hgrid_lines"):
                    y0, y1 = self.coords(item)[::2]
                    self.coords(item, coords_viewport[0], y0, lsup_x, y1)
            ...
            for item in items:
                self.move(item, -dx, 0)
            viewport_x0 = look.cell_containing_coords(linf_x + 1, 0, viewport, coords_viewport)[0]
            ...
            pass
        if ptx0:
            viewport_x1 = look.cell_containing_coords(winfo_width, 0, (viewport_x0, viewport[1]), coords_viewport)[0]
            viewport_x0 = viewport_x0 if isViewportOrig else ctx.viewport_q1[0]
            ctx.viewport_q1 = (viewport_x0, ctx.viewport_q1[1],viewport_x1, ctx.viewport_q1[3])
            pass

        if deltay and abs(deltay) >= winfo_height:
            # All the cell information and row headings need to be updated
            items = self.find_enclosed(clinf_x - 1, coords_viewport[1] - 1, gx1 + 1, gy1 + 1)
            self.delete(*items)
            viewport_y0 = y
            ...
            items = self.find_enclosed(clinf_x - 1, coords_viewport[1] - 1,  gx1 + 1, linf_y + 1)
            for item in items:
                self.move(item, 0, -dy)

            # Resize the vertical gridlines
            if gy1 != lsup_y:
                for item in self.find_withtag("vgrid_lines"):
                    x0, x1 = self.coords(item)[1::2]
                    self.coords(item, x0, clinf_y, x1, lsup_y)
            ...
            viewport_y0 = look.cell_containing_coords(0, linf_y + 1, viewport, coords_viewport)[1]
            pty0 = clinf_x, lsup_y - dy
        if pty0:
            viewport_y1 = look.cell_containing_coords(0, winfo_height, (viewport_x0, viewport_y0), coords_viewport)[1]
            viewport_y0 = viewport_y0 if isViewportOrig else ctx.viewport_q1[1]
            ctx.viewport_q1 = (ctx.viewport_q1[0], viewport_y0, ctx.viewport_q1[2], viewport_y1)
        if ptx0:
            if ptx1 is None:
                ptx1 = look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
            area = ptx0 + ptx1
            self.tag_area(*area, tag="invalid_area")
        if pty0:
            if pty1 is None:
                pty1 = look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
            area = pty0 + pty1
            self.tag_area(*area, tag="invalid_area")
        return ptx0 or pty0
    ...
    def move_viewport(self, x, y):
        if self.map_cell_to_coords(x, y):
    ...
    def validate_areas(self):
        ...
        while iareas:
            ...
            self.delete(item)
            if cx0 < ctx.coords_vportq3[0]:
                # Rows to draw
                area = cx0, cy0, ctx.coords_vportq3[0], cy1
                self.tag_area(*area, tag="rows_to_draw")
                cx0 = ctx.coords_vportq3[0]
            if cy0 < ctx.coords_vportq3[1]:
                # Columns to draw
                area = cx0, cy0, cx1, ctx.coords_vportq3[1]
                self.tag_area(*area, tag="cols_to_draw")
                cy0 = ctx.coords_vportq3[1]
    ...
    def redraw_headings(self):
        look = self.look
        ctx = look.sht_ctx
        cx0, cy0 = ctx.coords_vportq3
        cx1, cy1 = look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
        area = (cx0, 0, cx1, cy0)
    ...
    def setGUI(self):
        ...
        # Draw the background
        linf_coordx, linf_coordy = ctx.coords_vportq3[0] - COL_CELLS_WIDTH, ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT
        lsup_coordx, lsup_coordy = look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
        ...
        for item in self.find_withtag("cols_to_draw"):
            ...
            while cx0 < cx1 and xcell < MAX_COLS:
                nquadrant = look.cell_quadrant(cx0, cy1, isCoord=True)
                orig, coords_orig = look.quadrant_data(nquadrant)
                xcell = self.look.cell_containing_coords(cx0, cy0, orig, coords_orig)[0]
                y0, y1 = cy0, cy1
                x0, x1 = self.look.cell_coordinates(xcell, 0, orig, coords_orig)[::2]
                self.create_rectangle(x0, y0, x1, y1, fill="green", outline="black", tags="column")
                # Draw cell headings
                col_tag = self.format_header(xcell, axis=1)
                self.draw_cell_content((x0, y0, x1, y1), col_tag, fill="white", tags="columns_tag")
                # Draw vertical lines
        ...
        for item in self.find_withtag("rows_to_draw"):
            ...
            while cy0 < cy1 and ycell < MAX_ROWS:
                nquadrant = look.cell_quadrant(cx1, cy0, isCoord=True)
                orig, coords_orig = look.quadrant_data(nquadrant)
                ycell = look.cell_containing_coords(cx0, cy0, orig, coords_orig)[1]
                x0, x1 = cx0, cx1
                y0, y1 = self.look.cell_coordinates(0, ycell, orig, coords_orig)[1::2]
                self.create_rectangle(x0, y0, x1, y1, fill="green", outline="black", tags="row")
                # Draw cell headings
                row_tag = self.format_header(ycell, axis=0)
                self.draw_cell_content((x0, y0, x1, y1), row_tag, fill="white", tags="rows_tag")
    ...
    def show_ws_elements(self):
        ...
        if bflag1 := ctx.viewport_q3[0] != ctx.viewport_q3[1]:
            quadrants.append(4)
        if bflag2 := ctx.viewport_q3[1] != ctx.viewport_q3[3]:
            quadrants.append(2)
        if bflag1 and bflag2:
            quadrants.append(3)
        ...
        for nquadrant in quadrants:
            ...
            try:
                sel_x0, sel_y0, sel_x1, sel_y1 = clip_rectangle(*ctx.selected_cells, clipping_rect)
            except Exception as e:
                continue
            area_sel = self.area_coordinates(sel_x0, sel_y0, sel_x1, sel_y1)
            self.create_rectangle(*area_sel, fill="lightblue", outline="black", tags="selected_cells")
        ...
        nquadrant = self.cell_quadrant(*ctx.active_cell, isCoord=False)
        clipping_rect, _ = self.look.quadrant_data(nquadrant)
        try:
            sel_x0, sel_y0, sel_x1, sel_y1 = clip_rectangle(*ctx.active_cell, *ctx.active_cell, clipping_rect)
            area_sel = self.area_coordinates(sel_x0, sel_y0, sel_x1, sel_y1)
            self.create_rectangle(*area_sel, fill="yellow", outline="black", tags="active_cell")
        ...
        old_selected = set()
        old_selected.update(self.find_withtag("row_selected"))
        old_selected.update(self.find_withtag("col_selected"))
        
        new_selected = set()
        rx0, rx1 = ctx.coords_vportq3[0] - COL_CELLS_WIDTH, ctx.coords_vportq3[0]
        ry0, ry1 = ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT, ctx.coords_vportq3[1]
        sel_x0, sel_y0, sel_x1, sel_y1 = self.area_coordinates(*ctx.selected_cells)
        new_selected.update(srow for srow in self.find_enclosed(rx0 - 1, sel_y0 - 1, rx1 + 1, sel_y1 + 1) if 'row' in self.gettags(srow))
        new_selected.update(scol for scol in self.find_enclosed(sel_x0 - 1, ry0 - 1, sel_x1 + 1, ry1 + 1) if 'column' in self.gettags(scol))

        to_remove = old_selected - new_selected
        to_add = new_selected - old_selected
        for item in to_remove:
            tag = 'col_selected' if 'column' in self.gettags(item) else 'row_selected'
            self.dtag(item, tag)
            self.itemconfigure(item, fill="green")

        for item in to_add:
            tag = 'col_selected' if 'column' in self.gettags(item) else 'row_selected'
            self.addtag_withtag(tag, item)
            self.itemconfigure(item, fill="blue")
    ...
    def set_freeze_lines(self):
        ctx = self.look.sht_ctx
        coord_acell_x, coord_acell_y = ctx.coords_vportq1
        winfo_width, winfo_height = self.efective_width(), self.efective_height()
        self.delete("freeze_line")
        if coord_acell_y != ctx.coords_vportq3[1]:
            linf_x = ctx.coords_vportq3[0] - 2 * COL_CELLS_WIDTH
            self.create_line(linf_x, coord_acell_y, winfo_width, coord_acell_y, fill="black", tags="freeze_line")
        
        if coord_acell_x != ctx.coords_vportq3[0]:
            linf_y = ctx.coords_vportq3[1] - 2 * ROW_CELLS_HEIGHT
            self.create_line(coord_acell_x, linf_y, coord_acell_x, winfo_height, fill="black", tags="freeze_line")
        # self.show_cell(*ctx.viewport_q1[:2])
    ...
    def set_rows_height(self, height):
        ...
        clinf_x = ctx.coords_vportq3[0] - COL_CELLS_WIDTH
        vplsup_x0, vplsup_y0 = self.look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
        ...
        vplsup_x1, vplsup_y1 = self.look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
    ...
    def insert_rows(self):
        ...
        to_delete = (clinf_x - 1, linf_y - 1, ctx.coords_vportq3[0] + 1, vplsup_y1 + 1)
        self.delete(*self.find_enclosed(*to_delete))

        area = (clinf_x, linf_y, ctx.coords_vportq3[0], vplsup_y1)
        self.tag_area(*area, tag="invalid_area")
    ...
    def delete_rows(self):
        ...
        area = (clinf_x, linf_y, ctx.coords_vportq1[0] + 1, vplsup_y1 + 1)
        self.delete(*self.find_enclosed(*area))
        area = (clinf_x, linf_y, ctx.coords_vportq1[0], vplsup_y1)
        self.tag_area(*area, tag="invalid_area")
        logging.debug(f"Invalidated area: {self.area_cells(*area)}")
    ...
    def set_cols_width(self, width):
        ...
        clinf_y = ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT
        vplsup_x0, vplsup_y0 = self.look.cell_coordinates(*ctx.viewport_q1[2:])[2:]
    ...
    def insert_columns(self):
        ...
        to_delete = (linf_x - 1, clinf_y - 1, vplsup_x1 + 1, ctx.coords_vportq3[1] + 1)
        self.delete(*self.find_enclosed(*to_delete))

        area = linf_x, clinf_y, vplsup_x1, ctx.coords_vportq3[1]
        self.tag_area(*area, tag="invalid_area")
    ...
    def delete_columns(self):
        ...
        area = (linf_x - 1, clinf_y - 1, vplsup_x1 + 1, ctx.coords_vportq3[1] + 1)
        self.delete(*self.find_enclosed(*area))
        area = (linf_x, clinf_y, vplsup_x1, ctx.coords_vportq3[1])
        self.tag_area(*area, tag="invalid_area")
        logging.debug(f"Invalidated area: {self.area_cells(*area)}")
    ...
    def redraw_sheet(self, event=None, width=None, height=None):
        ...
        if self.find_withtag("background"):
            ...
            bgc_x0, bgc_x1 = ctx.coords_vportq3[0] - COL_CELLS_WIDTH, ctx.coords_vportq3[0]
            bgc_y0, bgc_y1 = ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT, ctx.coords_vportq3[1]
        ...
        ctx.viewport_q1 = ctx.viewport_q1[:2] + (viewport_x1, viewport_y1)

        clsup_x, clsup_y =self.look.cell_coordinates(viewport_x1, viewport_y1)[2:]
        ...
        if not bflag:
            # Columns to draw
            if bgc_x1 < clsup_x:
                area = (bgc_x1, ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT, clsup_x, ctx.coords_vportq3[1])
                self.tag_area(*area, tag="invalid_area")
                logging.debug(f"Invalidated area: {self.area_cells(*area)}")
            else:
                area = (clsup_x, ctx.coords_vportq3[1] - ROW_CELLS_HEIGHT, bgc_x1, ctx.coords_vportq3[1])
                self.delete(*self.find_enclosed(*area))
            # Rows to draw
            if bgc_y1 < clsup_y:
                area = (ctx.coords_vportq3[0] - COL_CELLS_WIDTH, bgc_y1, ctx.coords_vportq3[0], clsup_y)
                self.tag_area(*area, tag="invalid_area")
                logging.debug(f"Invalidated area: {self.area_cells(*area)}")
            else:
                area = (ctx.coords_vportq3[0] - COL_CELLS_WIDTH, clsup_y, ctx.coords_vportq3[0], bgc_y1)
                self.delete(*self.find_enclosed(*area))
            ...
            if bgc_x1 < clsup_x:
                if bgc_y1 - bgc_y0 > ROW_CELLS_HEIGHT:  
                    areas.append((bgc_x1, ctx.coords_vportq3[1], clsup_x, bgc_y1))
            else:
                area = (clsup_x - 1, ctx.coords_vportq3[1] -1, bgc_x1 + 1, bgc_y1 + 1)
                self.delete(*self.find_enclosed(*area))
                
            if bgc_y1 < clsup_y:
                if bgc_x1 - bgc_x0 > COL_CELLS_WIDTH:
                    areas.append((ctx.coords_vportq3[0], bgc_y1, bgc_x1, clsup_y))
            else:
                area = (ctx.coords_vportq3[0] - 1, clsup_y - 1, bgc_x1 + 1, bgc_y1 + 1)
                self.delete(*self.find_enclosed(*area))
            if areas:
                for area in areas:
                    self.tag_area(*area, tag="invalid_area")
    ...
    def offset_acell(self, dx, dy, state):
        ...
        if ctx.flags & SheetState.FREEZE is SheetState.NONE:
            if ctx.selected_cells[::2] == (1, MAX_COLS):
               xin, yin = ctx.viewport_q1[0], ctx.selected_cells[1::2][int(dy > 0)]
            elif ctx.selected_cells[1::2] == (1, MAX_ROWS):
               xin, yin = ctx.selected_cells[::2][int(dx > 0)],ctx.viewport_q1[1]
        nquadrant = self.cell_quadrant(xin, yin, isCoord=False)
        if (xin >= orig[0] and yin >= orig[1]) and nquadrant != 3:
            if nquadrant == 2:
                yin = ctx.viewport_q1[1]
            elif nquadrant == 4:
                xin = ctx.viewport_q1[0]
    ...
    def show_cell(self, xin, yin):
        ...
        viewport_x0, viewport_y0, viewport_x1, viewport_y1 = ctx.viewport_q1
        # lsup_coordx, lsup_coordy = self.look.cell_coordinates(viewport_x1, viewport_y1)[2:]
        # if lsup_coordx > winfo_width and xin >= viewport_x1:
        lsup_x = self.cell_containing_coords(winfo_width, 0)[0]
        if xin >= lsup_x:
            xright = self.look.cell_coordinates(xin, 0)[2]
            x = self.cell_containing_coords(xright - (winfo_width - ctx.coords_vportq1[0]), 0)[0]
            xright = self.look.cell_coordinates(x, 0)[2]
        ...
        if  yin >= lsup_y:
            ybottom = self.look.cell_coordinates(0, yin)[3]
            y = self.cell_containing_coords(0, ybottom - (winfo_height - ctx.coords_vportq1[1]))[1]
            ybottom = self.look.cell_coordinates(0, y)[3]
    ...
    def toggle_headings(self):
        ...
        ctx = self.look.sht_ctx
        if ctx.flags & SheetState.HEADINGS:
            dx, dy = -COL_CELLS_WIDTH, -ROW_CELLS_HEIGHT
        else:
            dx, dy = COL_CELLS_WIDTH, ROW_CELLS_HEIGHT
        lsup_x, lsup_y = self.look.cell_coordinates(ctx.viewport_q1[2], ctx.viewport_q1[3])[2:]
        ctx.coords_vportq1 = (ctx.coords_vportq1[0] + dx, ctx.coords_vportq1[1] + dy)
        ctx.coords_vportq3 = linf_x, linf_y = (ctx.coords_vportq3[0] + dx, ctx.coords_vportq3[1] + dy)
        items = self.find_enclosed(-linf_x - 1, -linf_y - 1, lsup_x + 1, lsup_y + 1)
        if ctx.flags & SheetState.FREEZE:
            items += self.find_withtag("freeze_line")
        for item in items:
            self.move(item, dx, dy)
        ctx.flags ^= SheetState.HEADINGS

    def toggle_gridlines(self):
        """Toggles the visibility of gridlines."""
        ctx = self.look.sht_ctx
        if ctx.flags & SheetState.GRIDLINES:
        ...
        ctx.flags ^= SheetState.GRIDLINES
    
    def freeze_panes(self, *coords):
        ctx = self.look.sht_ctx
        if not coords:
            coords = ctx.active_cell
        x0, y0, x1, y1 = ctx.viewport_q3
        coord_acx, coord_acy = self.look.cell_coordinates(*coords)[:2]
        if coords[0] != ctx.viewport_q1[0]:
            x0, x1 = ctx.viewport_q1[0], coords[0]
            ctx.coords_vportq1 = coord_acx, ctx.coords_vportq1[1]
        if coords[1] != ctx.viewport_q1[1]:
            y0, y1 = ctx.viewport_q1[1], coords[1]
            ctx.coords_vportq1 = ctx.coords_vportq1[0], coord_acy
        ctx.viewport_q3 = (x0, y0, x1, y1)
        ctx.viewport_q1 = *coords, *ctx.viewport_q1[2:]
        self.set_freeze_lines()
        self.xview_moveto(0.0)
        self.yview_moveto(0.0)
        ctx.flags ^= SheetState.FREEZE

    def unfreeze_panes(self):
        ctx = self.look.sht_ctx
        self.move_viewport(*ctx.viewport_q3[2:])
        ctx.coords_vportq1 = ctx.coords_vportq3
        ctx.viewport_q1 = *ctx.viewport_q3[:2], *ctx.viewport_q1[2:]
        ctx.viewport_q3 = 1, 1, 1, 1
        items = self.find_withtag("freeze_line")
        self.delete(*items)
        ctx.flags ^= SheetState.FREEZE
    ...
    def on_key_press(self, event):
        ...
        if event.keysym == "Home":
            ...
            viewport_x0, viewport_y0 = ctx.viewport_q1[:2]
            with self.pivot_point(isActiveCell=not isShiftPressed) as pivot:
                pivot.x = viewport_x0 = ctx.viewport_q3[2]
                if isCtrlPressed:
                    viewport_y0 = ctx.viewport_q3[3]
                    pivot.y = viewport_y0 
            self.event_generate("<<SelectedCellsChanged>>")
            self.move_viewport(viewport_x0, viewport_y0)
            self.xview_moveto(viewport_x0)
            ...
            with self.pivot_point(isActiveCell=not event.state & SHIFT_PRESSED) as pivot:
                coord_pivot_x, coord_pivot_y = self.look.cell_coordinates(pivot.x, pivot.y)[:2]
                nquadrant = self.cell_quadrant(pivot.x, pivot.y, isCoord=False)
                if event.state & ALT_PRESSED:
                    if nquadrant in (4, 3):
                        coord_pivot_x = ctx.coords_vportq1[0]
                    self.xview('scroll', -1 if event.keysym == 'Prior' else 1, 'pages')
                else:
                    if nquadrant in (2, 3):
                        coord_pivot_y = ctx.coords_vportq1[1]
            ...
            if ctx.selected_cells[:2] != ctx.selected_cells[2:]:
                sel_x0, sel_y0, sel_x1, sel_y1 = ctx.selected_cells
                acell_x0, acell_y0 = ctx.active_cell
                ...
                ctx.active_cell = (acell_x0, acell_y0)
    ...
    def on_mouse_click(self, event):
        ...
        if event.x < COL_CELLS_WIDTH and event.y < ROW_CELLS_HEIGHT:
            ctx.selected_cells = (1, 1, MAX_COLS, MAX_ROWS)
            ctx.active_cell = ctx.viewport_q1[:2]
            self.show_ws_elements()
            return "break"
        ...
        if row_clk := event.x < COL_CELLS_WIDTH: # and event.y >= ROW_CELLS_HEIGHT:
            sel_y0, sel_y1 = ctx.selected_cells[1::2]
            ctx.selected_cells = 1, sel_y0, MAX_COLS, sel_y1
            if not event.state & SHIFT_PRESSED:
                ctx.active_cell = (ctx.viewport_q1[0], clk_y)
        elif col_clk := event.y < ROW_CELLS_HEIGHT: # and event.x >= COL_CELLS_WIDTH:
            sel_x0, sel_x1 = ctx.selected_cells[::2]
            ctx.selected_cells = sel_x0, 1, sel_x1, MAX_ROWS
            if not event.state & SHIFT_PRESSED:
                ctx.active_cell = (clk_x, ctx.viewport_q1[1])
    ...
    def on_mouse_drag(self, event):
        ...
        if self.f_drag:
            ...
            if event_x >= COL_CELLS_WIDTH and event_y < ROW_CELLS_HEIGHT:
                ...
                if ctx.selected_cells[1::2] != (1, MAX_ROWS):
    ...
    def ymin_fraction(self):
            ...
            min_fraction = 1 - (y1 - y0) / (MAX_ROWS - ctx.viewport_q3[3])
            return min_fraction
    ...
    def yview(self, *args):
        ...
        if not args:
            ...
            viewport_y0, viewport_y1 = ctx.viewport_q1[1::2]
            denom = MAX_ROWS - ctx.viewport_q3[3]
            first = (viewport_y0 - ctx.viewport_q3[3]) / denom
            first = min(first, min_fraction)
            last = (viewport_y1 - ctx.viewport_q3[3]) / denom if first < min_fraction else 1.0
            return first, last
            ...
            if direction == 'units':
                ...
                if keysym == "Next":
                    ...
                    ytop, ybottom = self.look.cell_coordinates(0, viewport_y0)[1::2]
    ...
    def yview_moveto(self, fraction):
        ...
        match fraction:
            ...
            case _:
                ...
                viewport_y0 = int(fraction * (MAX_ROWS - ctx.viewport_q3[3]) + ctx.viewport_q3[3])

        viewport_x0 = ctx.viewport_q1[0]
        viewport_y0 = max(ctx.viewport_q3[3], min(MAX_ROWS, viewport_y0))
        self.move_viewport(viewport_x0, viewport_y0)
    ...
    def xmin_fraction(self):
        ...
        min_fraction = 1 - (x1 - x0) / (MAX_COLS - ctx.viewport_q3[2])
        return min_fraction
    ...
    def xview(self, *args):
        ...
        if not args:
            ...
            viewport_x0, viewport_x1 = ctx.viewport_q1[::2]
            denom = MAX_COLS - ctx.viewport_q3[2]
            first = (viewport_x0 - ctx.viewport_q3[2]) / denom
            first = min(first, min_fraction)
            last = (viewport_x1 - ctx.viewport_q3[2]) / denom if first < min_fraction else 1.0
            return first, last
            ...
            if direction == 'units':
                ...
                if keysym == "Next":
                    ...
                    xtop, xbottom = self.look.cell_coordinates(viewport_x0, 0)[::2]
    ...
    def xview_moveto(self, fraction):
        ...
        match fraction:
            ...
            case _:
                ...
                viewport_x0 = int(fraction * (MAX_COLS - ctx.viewport_q3[2]) + ctx.viewport_q3[2])

        viewport_y0 = ctx.viewport_q1[1]
        viewport_x0 = max(ctx.viewport_q3[2], min(MAX_COLS, viewport_x0))
        self.move_viewport(viewport_x0, viewport_y0)
...
class SheetViewer(tk.Tk):
    ...
    def on_selected_cells_changed(self, event):
        wdg = event.widget
        ctx = wdg.look.sht_ctx
        selected_cells = ctx.selected_cells
    ...
    def setGui(self):   
        ...
        self.sheetui = sheetui = SheetUI(frame, cell_content_gen=test_content_gen, name='sheetui', bg=GRID_COLOR, 
